# backtest/plugins/m1_market_structure/storage.py
"""
Storage handler for M1 Market Structure calculation results.
...
class M1MarketStructureStorage:
    """Handles storage operations for M1 Market Structure results"""
    
    @staticmethod
    def prepare_storage_data(uid: str, signal_data: Dict) -> Dict[str, Any]:
        """
        Prepare data for storage in bt_m1_market_structure table.
    ...
    async def store(supabase_client, uid: str, signal_data: Dict) -> bool:
        ...
        try:
            # Prepare data
            storage_data = M1MarketStructureStorage.prepare_storage_data(uid, signal_data)
            
            # Upsert to handle re-runs
            response = supabase_client.table('bt_m1_market_structure').upsert(storage_data).execute()
            
            if response.data:
                logger.debug(f"Successfully stored M1 Market Structure results for {uid}")
                return True
            else:
                logger.error(f"Failed to store M1 Market Structure results for {uid}: No data returned")
                return False
                
        except Exception as e:
            logger.error(f"Error storing M1 Market Structure results for {uid}: {e}")
            return False