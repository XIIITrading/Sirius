-- backtest/plugins/m1_ema/schema.sql
-- Table schema for M1 EMA calculation results

CREATE TABLE public.bt_m1_ema (
  ...
  reason text null,
  constraint bt_m1_ema_pkey primary key (uid),
  constraint bt_m1_ema_uid_fkey foreign key (uid) references bt_index (uid) on delete cascade
