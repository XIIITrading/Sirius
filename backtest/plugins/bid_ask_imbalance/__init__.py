...
from .plugin import BidAskImbalancePlugin

# Configure logging
logger = logging.getLogger(__name__)

# Create plugin instance
_plugin = BidAskImbalancePlugin()
...
async def run_analysis(symbol: str, entry_time: datetime, direction: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(f"Bid/Ask Imbalance analysis complete for {symbol} at {entry_time}")
        return result
...
# Optional: Export performance mode setter
def set_performance_mode(enabled: bool):
    """Enable or disable performance mode"""
    _plugin.set_performance_mode(enabled)