...
from .plugin import M1StatisticalTrendPlugin

# Configure logging
logger = logging.getLogger(__name__)

# Create plugin instance
_plugin = M1StatisticalTrendPlugin()

# Export the main interface
async def run_analysis(symbol: str, entry_time: datetime, direction: str) -> Dict[str, Any]:
    """
    Run 1-Minute Statistical Trend analysis.
    ...
    try:
        ...
        logger.info(f"1-Min Statistical Trend analysis complete for {symbol} at {entry_time}")
        return result
        
    except Exception as e:
        logger.error(f"Error in 1-Min Statistical Trend analysis: {e}")
        # Return error result
        return {
