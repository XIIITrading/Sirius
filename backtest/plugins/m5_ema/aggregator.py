# backtest/plugins/m5_ema/aggregator.py
"""Aggregator for M5 EMA plugin - converts 1-minute bars to 5-minute bars"""
...
class M5EMAggregator:
    """
    Handles aggregation of 1-minute bars to 5-minute bars.
    ...
    def aggregate(self, bars_1min: pd.DataFrame) -> pd.DataFrame:
        ...
        # Perform aggregation to 5-minute bars
        bars_5min = self._aggregate_to_5min(data)
        
        logger.info(f"Aggregated {len(data)} 1-minute bars to {len(bars_5min)} 5-minute bars")
    ...
    def _aggregate_to_5min(self, bars_1min: pd.DataFrame) -> pd.DataFrame:
        ...
        bars_5min = bars_1min.resample('5T', label='right', closed='right').agg({
            'open': 'first',
            'high': 'max',
            'low': 'min',
            'close': 'last',
        ...
        if 'vwap' not in bars_5min.columns or bars_5min['vwap'].isna().all():
