# backtest/plugins/m15_market_structure/adapter.py
"""
M15 Market Structure Adapter for backtesting.
Uses shared data cache from the engine and fractal-based market structure analysis on 15-minute timeframe.
"""
...
from .aggregator import M15MarketStructureAggregator

logger = logging.getLogger(__name__)


class M15MarketStructureBackAdapter(CalculationAdapter):
    """
    Adapter for 15-minute Market Structure backtesting.
    ...
    def __init__(self, 
        ...
        """
        Initialize M15 market structure adapter.
        
        Args:
            fractal_length: Number of bars on each side for fractal detection
            buffer_size: Number of 15-minute candles to maintain in buffer
            min_candles_required: Minimum candles needed for analysis
        """
        super().__init__(
            calculation_class=M15MarketStructureAnalyzer,
            config={
                'fractal_length': fractal_length,
                'buffer_size': buffer_size,
                'min_candles_required': min_candles_required
            },
            name="15-Min Market Structure"
        )
        
        # Data requirements
        self.requires_trades = False
        self.requires_quotes = False
        self.warmup_periods = max(min_candles_required * 15, fractal_length * 30 + 15)  # Convert to minutes
        
        # Shared data cache reference
        self.data_cache = None
        
        # Initialize aggregator
        self.aggregator = M15MarketStructureAggregator()
        
        logger.info(f"M15MarketStructureBackAdapter initialized with fractal_length={fractal_length}, "
    ...
    def get_data_requirements(self) -> Dict:
        ...
        return {
            'bars': {
                'timeframe': '1min',
                'lookback_minutes': 1800  # 30 hours of 1-minute data for 15-min aggregation
            },
    ...
    def feed_historical_data(self, data: pd.DataFrame, symbol: str) -> None:
        ...
        # Use aggregator to create 15-minute bars
        bars_15min = self.aggregator.aggregate(bars_1min)
        
        if bars_15min.empty:
            logger.warning(f"No valid 15-minute bars after aggregation")
            return
        ...
        logger.info(f"Processing {len(bars_15min)} historical 15-minute bars for {symbol}")
        
        # Convert to candle format and process
        candles = []
        for timestamp, row in bars_15min.iterrows():
            ...
            candle_dict = {
                't': timestamp,  # M15MarketStructureAnalyzer expects 't' key
                'o': float(row['open']),
                'h': float(row['high']),
                'l': float(row['low']),
                'c': float(row['close']),
        ...
        logger.info(f"Prepared {len(candles)} 15-minute candles for processing")
