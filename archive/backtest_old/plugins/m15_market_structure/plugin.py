# backtest/plugins/m15_market_structure/plugin.py
"""
M15 Market Structure Plugin
Complete self-contained plugin for 15-minute fractal-based market structure analysis.
"""

from typing import Dict, Any, Type
from plugins.base_plugin import BacktestPlugin
from .adapter import M15MarketStructureBackAdapter
from .storage import M15MarketStructureStorage


class M15MarketStructurePlugin(BacktestPlugin):
    """Plugin for 15-minute market structure calculation using fractals"""
    ...
    def adapter_class(self) -> Type:
        return M15MarketStructureBackAdapter
    
    def get_adapter_config(self) -> Dict[str, Any]:
        """Configuration for M15 Market Structure adapter"""
        return {
            'fractal_length': 2,       # Even fewer bars for 15-min fractal detection
            'buffer_size': 60,         # 60 15-min candles = 900 minutes (15 hours)
            'min_candles_required': 10 # Minimum for valid signal
        }
    ...
    def storage_table(self) -> str:
        return "bt_m15_market_structure"
    ...
    async def store_results(self, supabase_client, uid: str, signal_data: Dict) -> bool:
        ...
        return await M15MarketStructureStorage.store(supabase_client, uid, signal_data_clean)


# Export plugin instance
plugin = M15MarketStructurePlugin()