# modules/calculations/market-structure/m5_market_structure.py
"""
...
class M5MarketStructureAnalyzer:
    ...
    def __init__(self, 
                 fractal_length: int = 5,      # Fewer bars for 5-min timeframe
...
# ============= TEST FUNCTION =============
async def test_m5_market_structure_analyzer():
    """Test M5 market structure analyzer with real-time websocket data"""
    import sys
    import os
    ...
    print("=== M5 MARKET STRUCTURE ANALYZER TEST ===")
    print("Analyzing market structure using fractals on 5-minute charts")
    print(f"Current UTC time: {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}\n")
    
    # Test configuration
    TEST_SYMBOLS = ['AAPL', 'TSLA', 'SPY']
    TEST_DURATION = 600  # 10 minutes to see more 5-min candles
    USE_HISTORICAL = True  # Load historical data first
    
    # Create analyzer with fractal length of 3 for 5-minute
    ...
    if USE_HISTORICAL:
        ...
        for symbol in TEST_SYMBOLS:
            # Get last 100 5-minute candles (500 minutes of data)
            ...
            try:
                ...
                if candles:
                    print(f"\n{symbol}: Loading {len(candles)} historical 5-minute candles")
    ...
    async def handle_candle(data: Dict):
        ...
        # For 5-minute aggregates, check if this is a complete candle
        ...
        if signal:
            ...
            print(f"{emoji} {signal.symbol} - {signal.signal} Market Structure (5-MIN) {structure_emoji}")
    ...
    try:
        ...
        # Subscribe to aggregates with 5-minute multiplier
        ...
        print("✓ Subscribed successfully to 5-minute data")
        
        print(f"\n⏰ Running for {TEST_DURATION} seconds...")
        print("Waiting for 5-minute market structure changes...\n")
        ...
        if signal_history:
            ...
            # Show current state
            print(f"\n📍 Current 5-Minute Market Structure:")
            for symbol in TEST_SYMBOLS:
...
if __name__ == "__main__":
    print(f"Starting M5 Market Structure Analyzer at {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}")
    print("This module detects market structure changes using fractals on 5-minute charts")
    print("All timestamps are in UTC\n")
    
    asyncio.run(test_m5_market_structure_analyzer())