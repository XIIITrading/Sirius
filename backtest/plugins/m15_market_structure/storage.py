# backtest/plugins/m15_market_structure/storage.py
"""
Storage handler for M15 Market Structure calculation results.
Handles Supabase storage operations for 15-minute market structure analysis.
"""
...
class M15MarketStructureStorage:
    """Handles storage operations for M15 Market Structure results"""
    
    @staticmethod
    def prepare_storage_data(uid: str, signal_data: Dict) -> Dict[str, Any]:
        """
        Prepare data for storage in bt_m15_market_structure table.
    ...
    async def store(supabase_client, uid: str, signal_data: Dict) -> bool:
        """
        Store M15 Market Structure results to Supabase.
        ...
        try:
            # Prepare data
            storage_data = M15MarketStructureStorage.prepare_storage_data(uid, signal_data)
            
            # Upsert to handle re-runs
            response = supabase_client.table('bt_m15_market_structure').upsert(storage_data).execute()
            
            if response.data:
                logger.debug(f"Successfully stored M15 Market Structure results for {uid}")
                return True
            else:
                logger.error(f"Failed to store M15 Market Structure results for {uid}: No data returned")
                return False
                
        except Exception as e:
            logger.error(f"Error storing M15 Market Structure results for {uid}: {e}")
            return False