# modules/calculations/market-structure/m1_market_structure.py
"""
...
class MarketStructureAnalyzer:
    """
    Fractal-based market structure analyzer for trend detection
    ...
    def process_candle(self, symbol: str, candle_data: Dict, 
        ...
        if symbol not in self.candles:
            ...
            self.high_fractals[symbol] = deque(maxlen=50)
            self.low_fractals[symbol] = deque(maxlen=50)
        ...
        if not is_complete:
            logger.debug(f"{symbol}: Skipping incomplete candle")
            return None
        ...
        logger.debug(f"{symbol}: Processed candle at {timestamp.strftime('%H:%M:%S UTC')} "
        ...
        if len(self.candles[symbol]) >= self.min_candles_required:
            ...
            if signal:
                logger.info(f"{symbol}: Generated {signal.signal} signal - {signal.structure_type}")
            return signal
        else:
    ...
    def _detect_fractals(self, symbol: str):
        """Detect fractal patterns in the candle buffer"""
        ...
        # We check historical fractals, not the most recent candle
        ...
        if is_high_fractal:
            ...
            if not existing:
                ...
                logger.debug(f"{symbol}: High fractal detected at {middle_high:.2f} "
        ...
        if is_low_fractal:
            ...
            if not existing:
                ...
                self.fractals_detected += 1
                logger.debug(f"{symbol}: Low fractal detected at {middle_low:.2f} "
                           f"({middle_candle.timestamp.strftime('%H:%M:%S UTC')})")
    ...
    def _generate_signal(self, symbol: str, direction: str, structure_type: str,
        ...
        base_strength = 70 if structure_type == 'CHoCH' else 50
...
async def test_market_structure_analyzer():
    """Test market structure analyzer with real-time websocket data"""
    import sys
    import os
    ...
    print("Analyzing market structure using fractals")
    ...
    # Create analyzer with fractal length of 5
    ...
    if USE_HISTORICAL:
        ...
        for symbol in TEST_SYMBOLS:
            # Get last 200 1-minute candles for better fractal detection
    ...
    async def handle_candle(data: Dict):
        ...
        if signal:
            ...
            print(f"{emoji} {signal.symbol} - {signal.signal} Market Structure {structure_emoji}")
    ...
    try:
        ...
        print("Waiting for market structure changes...\n")
...
if __name__ == "__main__":
    print(f"Starting Market Structure Analyzer at {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}")
    print("This module detects market structure changes using fractals")
