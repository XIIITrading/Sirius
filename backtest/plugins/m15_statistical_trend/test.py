...
"""
Test for 15-Minute Statistical Trend Plugin
"""
...
async def test_statistical_trend(symbol: str, test_time: datetime, direction: str):
    ...
    try:
        ...
        print(f"Position Signal: {details['position_signal']}")
        ...
        if details['short_trend']:
            ...
            print(f"45-min (3 bars): {short.get('direction', 'N/A').upper()} "
                  f"(Strength: {short.get('strength', 0):.0f}%, "
                  f"VWAP: {short.get('vwap_position', 0):+.2f}%)")
        
        if details['medium_trend']:
            medium = details['medium_trend']
            print(f"75-min (5 bars): {medium.get('direction', 'N/A').upper()} "
                  f"(Strength: {medium.get('strength', 0):.0f}%, "
                  f"Score: {medium.get('score', 0):.3f})")
        
        if details['long_trend']:
            long = details['long_trend']
            print(f"150-min (10 bars): {long.get('direction', 'N/A').upper()} "
                  f"(Strength: {long.get('strength', 0):.0f}%, "
                  f"Score: {long.get('score', 0):.3f})")
        
        # Regime progression
        if 'regime_progression' in details:
            print(f"\n{details['regime_progression']}")
        
        # Signal history
        display_data = result['display_data']
        if 'signal_history' in display_data:
            history = display_data['signal_history']
            print(f"\nSIGNAL HISTORY (last {details['signals_analyzed']} 15-min bars):")
            print("-" * 95)
            
            # Headers
            headers = history['headers']
            print(f"{headers[0]:^10} | {headers[1]:^8} | {headers[2]:^15} | {headers[3]:^9} | "
                  f"{headers[4]:^6} | {headers[5]:^5} | {headers[6]:^19} | {headers[7]:^13}")
            print("-" * 95)
            
            # Rows
            for row in history['rows']:
                print(f"{row[0]:^10} | {row[1]:^8} | {row[2]:^15} | {row[3]:^9} | "
                      f"{row[4]:^6} | {row[5]:^5} | {row[6]:^19} | {row[7]:^13}")
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='15-Minute Statistical Trend Analysis Test'
    )
...
async def main():
    ...
    if args.time:
        ...
        # Default to 4 hours ago for 15-min data
        test_time = datetime.now(timezone.utc) - timedelta(hours=4)
