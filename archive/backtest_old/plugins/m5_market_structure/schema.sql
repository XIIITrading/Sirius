-- backtest/plugins/m5_market_structure/schema.sql
-- Table schema for M5 Market Structure calculation results

CREATE TABLE public.bt_m5_market_structure (
  ...
  timeframe character varying(20) null default '5-minute',
  reason text null,
  constraint bt_m5_market_structure_pkey primary key (uid),
  constraint bt_m5_market_structure_uid_fkey foreign key (uid) references bt_index (uid) on delete cascade
) tablespace pg_default;

-- Create index for faster queries on structure type
CREATE INDEX idx_bt_m5_market_structure_type ON public.bt_m5_market_structure(structure_type);

-- Create index for trend analysis
CREATE INDEX idx_bt_m5_market_structure_trend ON public.bt_m5_market_structure(current_trend);

-- Create index for timeframe queries
CREATE INDEX idx_bt_m5_market_structure_timeframe ON public.bt_m5_market_structure(timeframe);